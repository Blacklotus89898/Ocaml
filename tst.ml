...
let rec exp_parser i = 
  ...
  let rec applicative_exp_parser : exp Parser.t =
    let open Parser in
    atomic_exp_parser |*> fun first_exp ->
      many atomic_exp_parser |> map (fun exps ->
          List.fold_left (fun acc e -> Apply(acc, e)) first_exp exps)
  in 
  ...
  let rec negation_exp_parser : exp Parser.t =
    let open Parser in
  ...
  in
  let rec multiplicative_exp_parser i =
    let open Parser in
  ...
  (* Define the left associative operation parser using the operator parser. *)
    let additive_impl =
      left_assoc_op operator_parser multiplicative_exp_parser (fun acc op_func e -> op_func acc e)
    in
  ...
  let rec comparative_exp_parser : exp Parser.t =
    let open Parser in
    first_of [ 
...
let rec typ_infer (ctx : Context.t) (e : exp) : typ =
  ...
  | If (e', e1, e2) ->
      ...
      if typ_infer ctx e1 = typ_infer ctx e2
      then typ_infer ctx e2
      else raise TypeInferenceError
  ...
  | Apply (e1, e2) ->
      ...
      (match tau1 with
       | Arrow (tau11, tau12) ->
           if tau11 = tau2 then tau12
           else raise TypeInferenceError
       | _ -> raise TypeInferenceError)

  | Rec (f, Some t, e') ->  (*typ_infer (Context.extend ctx (f, t)) e'*)
...
let rec eval (e : exp) : exp =
  ...
  | Rec (f, tOpt, e') -> subst (Rec (f, None, e'), f) e' |> eval
...
let rec adv_typ_infer (ctx : Context.t) (e : exp) : typ =
  ...
  | LetComma (x, y, e1, e2) ->
      let t1 = adv_typ_infer ctx e1 in
      let ctx' = Context.extend (Context.extend ctx (x, t1)) (y, t1) in
  ...
  | Rec (f, None, e') ->
      ...
      let t' = adv_typ_infer ctx' e' in
      (match !a, !b with
       | Some ta, Some tb -> unify (Arrow (ta, tb)) t'; reset_print_typ (); Arrow (ta, tb)
